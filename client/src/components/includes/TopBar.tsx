import React, { useState } from 'react';
import { Icon } from 'semantic-ui-react';

const TopBar = (props: {
    courseId: string;
    user: FireUser;
    // A user's role: student, ta, or professor
    // We show TA's and Profs extra links
    role: string;
    // Whether we're in a "professor" view or "student" view
    // controls where "switch view" goes
    context: string;
}) => {
    const [showMenu, setMenu] = useState(false);

    const redirect = (href: string) => {
        document.location.href = href;
    };

    return (
        <div className="MenuBox" tabIndex={1} onBlur={() => setMenu(false)}>
            <header className="topBar">
                <div className="triggerArea" onClick={() => setMenu(!showMenu)}>
                    <div className="userProfile">
                        <img src={props.user.photoUrl} alt="User Avatar" />
                        {/* // TODO */}
                        <span className="name">
                            {`${props.user.firstName} ${props.user.lastName}`}
                        </span>
                    </div>
                </div>
            </header>
            {showMenu && (
                <React.Fragment>
                    <ul className="desktop logoutMenu" tabIndex={1} onClick={() => setMenu(false)}>
                        <li onMouseDown={() => redirect('/__auth/logout')}>
                            <span><Icon name="sign out" /></span>
                            Log Out
                        </li>
                        <li onMouseDown={() => window.open('https://goo.gl/forms/7ozmsHfXYWNs8Y2i1', '_blank')}>
                            <span><Icon name="edit" /></span>
                            Send Feedback
                        </li>
                        {props.role === 'professor' && (
                            <React.Fragment>
                                {props.context === 'professor'
                                    ? (
                                        <li onMouseDown={() => redirect(`/course/${props.courseId}`)}>
                                            <span><Icon name="sync alternate" /></span>
                                            Switch View
                                        </li>
                                    ) : (
                                        <li onMouseDown={() => redirect(`/professor/course/${props.courseId}`)}>
                                            <span><Icon name="sync alternate" /></span>
                                            Switch View
                                        </li>
                                    )
                                }
                            </React.Fragment>)
                        }
                    </ul>
                </React.Fragment>
            )}
        </div>
    );
};

export default TopBar;
