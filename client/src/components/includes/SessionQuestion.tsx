...
const UPDATE_QUESTION = gql`
  mutation UpdateQuestion($questionId: Int!, $status: String) {
    updateQuestionByQuestionId(
      input: { questionPatch: { status: $status }, questionId: $questionId }
    ) {
      clientMutationId
    }
  }
...
const UPDATE_LOCATION = gql`
  mutation UpdateLocation($questionId: Int!, $location: String) {
    updateQuestionByQuestionId(
      input: { questionPatch: { location: $location }, questionId: $questionId }
    ) {
      clientMutationId
    }
  }
...
class SessionQuestion extends React.Component {
  ...
  public handleUpdateLocation = (
    ...
    if (target.value.length <= LOCATION_CHAR_LIMIT) {
      ...
      setTimeout(() => {
        this.state.isEditingLocation = false;
      },         100);
    }
  }
  ...
  render() {
    ...
    return (
      <div className="QueueQuestions">
        ...
        <p
          className={
            'Order ' + (question.status === 'assigned' ? 'assigned' : '')
          }
        >
        ...
        <div className="QuestionInfo">
                  ...
                  <React.Fragment>
                    <span className="assigned">
                      ...
                      {question.userByAnswererId &&
                        ' to ' +
                          (question.userByAnswererId.userId ===
                          this.props.myUserId
                            ? 'you'
          ...
          <div className="Location">{this.props.isTA && question.location}</div>
          {(this.props.isTA ||
            this.props.includeBookmark ||
            this.props.includeRemove) && (
        ...
        <div className="BottomBar">
          ...
          <div className="Tags">
            {question.questionTagsByQuestionId.nodes.map(tag => (
              <SelectedTags
                ...
                <div className="TAButtons">
                  {question.status === 'unresolved' && (
                    <p
                      className="Begin"
                      onClick={e =>
                        this._onClick(e, updateQuestion, 'assigned')
                      }
                    >
                      Assign to Me
                    </p>
                  ...
                  {question.status === 'assigned' && (
                    <React.Fragment>
                      <p
                        className="Delete"
                        onClick={e =>
                          this._onClick(e, updateQuestion, 'no-show')
                        }
                      >
                        No show
                      </p>
                      <p
                        className="Done"
                        onClick={e =>
                          this._onClick(e, updateQuestion, 'resolved')
                        }
                      >
                        Done
                      </p>
