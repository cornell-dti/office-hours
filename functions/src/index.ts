...
exports.onQuestionUpdate = functions.firestore
    ...
    .onUpdate(async (change) => {
        ...
        if (numAssignedChange === 1 && newQuestion.timeAssigned !== undefined) {
            ...
            waitTimeChange = (newQuestion.timeAssigned.seconds - newQuestion.timeEntered.seconds) / newQuestion.position;
        }
        else if (numAssignedChange === -1 && prevQuestion.timeAssigned !== undefined) {
            // Subtract previous time addressed
            waitTimeChange = (prevQuestion.timeEntered.seconds - prevQuestion.timeAssigned.seconds) / prevQuestion.position;
        }
