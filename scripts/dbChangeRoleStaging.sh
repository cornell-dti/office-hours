...
echo '---------------'
echo 'This script will change the role of particular user in a particular course in staging for testing purposes. Make sure you have the right course id handy!'
echo ''
echo '---------------'
echo ''
...
echo "Are you sure you want to make ${email} a ${role} in course ${courseid} on staging? Enter \"yes\" to continue: "
read confirmation
if [ "$confirmation" != "yes" ]
then
    exit 1
...
echo "Changing the user's role in the staging database..."
...
echo 'Done! Refresh the course page on staging to see the changes :)'