...
type Props = {
    ...
    readonly showProfessorStudentView: boolean;
    setRemoveQuestionId: (questionId: string) => void;
};
...
const SessionQuestionsContainer = (props: Props) => {
    ...
    const newQuestionAssigned = () => {
        if (
            typeof props.course.isTimeLimit === "undefined" ||
            !props.course.isTimeLimit ||
            typeof props.course.timeLimit === "undefined" ||
            typeof props.course.timeWarning === "undefined"
        )
            return;
    ...
    const otherQuestions = shownQuestions.filter(
        (question) => question.status !== "assigned" && !assignedQuestions.includes(question)
    );
    ...
    return (
        <div className="SessionQuestionsWrapper">
            ...
            <div
                className={
                    "SessionQuestionsContainer splitQuestions" +
            ...
            >
                {!props.isTA && !myQuestion && props.isOpen && !props.isPaused && !props.haveAnotherQuestion ? (
                    props.course && props.session ? (
                        <AddQuestion
                            ...
                            showProfessorStudentView={props.showProfessorStudentView}
                        />
                    ) : (
                ...
                {!props.isTA && !myQuestion && props.isOpen && !props.isPaused && props.haveAnotherQuestion && (
                    <>
                        <div className="SessionClosedMessage">
                            You are holding a spot in another active queue. To join this queue, please retract your
                            question from the other queue!
                        </div>
                ...
                {shownQuestions && !props.isTA && myQuestion && (
                    <StudentMyQuestion
                ...
                {shownQuestions && shownQuestions.length > 0 && props.modality === "review" && (
                    <div className="discussionHeaderWrapper">
                        <div className="discussionQuestionsSlider">
                            <div
                                className={
                                    "discussionSliderSelector" + (filterByAnsweredQuestions ? " isSlidedRight" : "")
                                }
                            />
                            <div
