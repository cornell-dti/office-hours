...
def main():
    # Use quotations when passing in string arguments. For example,"Nidhi Soma" is the argument WITH quotes. Additionally, the csv file argument should include whole filename, including .csv . For example, "data.csv" if current directory, or if in subdirectiory then "src/data.csv".
    ...
    with open(filename, mode='r') as file:
        ...
        for lines in csvFile:
            ...
            if ',' in profnames:
                ...
                final = profnames[0].strip().split(' ')[1] + ' and Professor ' + profnames[1].strip().split(' ')[1]
            else:
                final = profnames.split(' ')
